...
namespace TravelerGuideApp.UnitTests
{
    ...
    public class LocationControllerFixture
    {
        ...
        {
            // Arrange
            _mockMediator.Setup(m => m.Send(It.IsAny<GetLocationsForCityQuery>(), It.IsAny<CancellationToken>())).Verifiable();
            // Act
            var controller = new LocationsController(_mockMapper.Object, _mockMediator.Object);
            await controller.GetAll();
            // Assert
            _mockMediator.Verify(x => x.Send(It.IsAny<GetLocationsForCityQuery>(), It.IsAny<CancellationToken>()), Times.Once());
